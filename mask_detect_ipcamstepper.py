...
url='http://192.168.1.47:8080/shot.jpg'     #Address of local IP Camera
...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        if mask > withoutMask:
            ...
            #green.on()
            stepper_status = 1
            from stepper_motor_clockwise import printImported
            printImported()
            time.sleep(2)
            stepper_status = 0
            from stepper_motor_counterclockwise import printImported2
            printImported2() 
        else:
